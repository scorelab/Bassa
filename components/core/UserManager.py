...
def check_approved(username, password):
    ...
    if db is not None:
        cursor = db.cursor()
        sql = \
            'SELECT * FROM user WHERE user_name=%s AND password=MD5(%s) AND approved=1;'
        cursor.execute(sql, (username, password))
        data = cursor.fetchone()
        db.close()
        if data == None:
...
def get_user(username):
    ...
    if db is not None:
        ...
        if data == None:
            ...
            return User(data['user_name'], data['password'], data['auth'
                        ], data['email'])
...
def update_user(user, username):
    ...
    if db is not None:
        cursor = db.cursor()
        sql = \
            'UPDATE user SET user_name=%s, auth=%s, email=%s WHERE user_name=%s;'
        try:
            cursor.execute(sql, (user.userName, user.auth, user.email,
                           username))
            db.commit()
        except MySQLdb.Error, e:
            db.rollback()
...
def get_heavy_users():
    ...
    if db is not None:
        ...
        MONTH = 60 * 60 * 24 * 30
        sql = \
            'SELECT user_name, sum(size) AS size FROM download WHERE completed_time > unix_timestamp(now()) - %s GROUP BY user_name ORDER BY size DESC LIMIT 10;' \
            % MONTH
        try:
            cursor.execute(sql)
            results = cursor.fetchall()
...
def check_if_bandwidth_exceeded(username):
    ...
    if db is not None:
        ...
        MONTH = 60 * 60 * 24 * 30
        sql = \
            'SELECT sum(size) AS sum FROM download WHERE completed_time > unix_timestamp(now()) - %s AND user_name=%s;'
        try:
